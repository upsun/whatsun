...
* `dep.getVersion(managerType string, name string)` -> `string`
...
* `dep.has(managerType string, pattern string)` -> `bool`
    - `managerType`: The manager type (`go`, `js` or `php`)
...
* `file.contains(filename string, substr string)` -> `bool`
...
* `file.glob(pattern string)` -> `list(string)`

### `file.isDir`
Check if a file exists and is a directory.

* `file.isDir(name string)` -> `bool`

### `file.read`
Read a file.

* `file.read(filename string)` -> `bytes`

### `json.queryString`
Query a byte string (e.g. file contents) using JQ.

* `json.queryString(contents bytes, query string)` -> `string`

### `version.parse`
Parse a semantic version into major, minor and patch components.

* `version.parse(version string)` -> `map(string, string)`
